...
async def _menu_keyboard(msg: types.Message, user_id=None):
    ...
    if await ClientId.objects.filter(user_id=user_id, deleted=False).aexists():
        ...
        keyboard.row(types.KeyboardButton(text=_(ONLINE_BUY, msg.bot.lang)), types.KeyboardButton(text=f"{_(ORDERS, msg.bot.lang)} ({await Order.objects.filter(with_online_buy=False, client__in=clientid.clients.all()).acount()})"))
    else:
